...
class UserRepository(CrudRepository[UserModel, UserCreate, UserUpdate]):
    ...
    # async def available(
    #     self,
    #     users_sid_list: list[UUID],
    # ) -> Sequence[UserModel]:
    #     query = select(self.model).where((self.model.sid.in_(users_sid_list)))
    #
    ...
    #
    # @logging_function_info(logger=logger)
    # async def all_opponents(
    #     self, current_user_sid: UUID, list_party_sid: List[UUID]
    # ) -> Sequence[UserModel]:
    #     query = (
    #         select(self.model)
    #         .distinct(self.model.sid)
    #         .join(PartyUserModel.user)
    #         .join(PartyUserModel.party)
    #         .join(PartyModel.game)
    #         .where(
    #             (PartyUserModel.user_sid != current_user_sid)
    #             & (PartyUserModel.party_sid.in_(list_party_sid))
    #         )
    #     )
    #     result = await self.db.execute(query)
    #
    ...
    #
    # @logging_function_info(logger=logger)
    # async def existence(self, list_sid: list[UUID]) -> Sequence[UserModel]:
    #     query = select(self.model).where(self.model.sid.in_(list_sid))
    #     result = await self.db.execute(query)
    #
    #     return result.scalars().all()
    #
